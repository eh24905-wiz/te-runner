...
| # | Finding | Do | Effort | Blast radius → guidance |
|---|---|---|---|---|
| 21 | 13 lab repos each carry an open PR `repin-v0.1.47` (pin v0.1.47, wrappers on `wizlab --check`) | merge them; play one lab end to end first (`te-wiz-sensor-101`) | S | Every PR passes `te-labkit-v2/scripts/runner-floor.py` at v0.1.47. A wrapper left in place stays correct, so the order of merges is free. |
